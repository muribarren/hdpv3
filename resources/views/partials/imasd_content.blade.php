<form method="POST" action="{{ route('procesar')}}" enctype="multipart/form-data">
    ...
    <div class="w-full border border-black rounded-lg p-6 mt-6">
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4" hidden>
            <input type="text" id="clase" name="clase" value="{{ $imasd->clase ?? '' }}"  maxlength="250"  {{ $participante->imasd != $loged_user->id ? 'readonly' : '' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            ...
            <label for="ensayo_hecho" class="text-sm text-gray-700 dark:text-gray-400">
                ...
                <input type="checkbox" id="ensayo_hecho" value = "1" {{ $participante->imasd != $loged_user->id ? 'disabled' : '' }} name="ensayo_hecho" @if ($imasd->ensayo_hecho) checked @endif  class="mr-2 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 " {{ old('ensayo_hecho') ? 'checked' : '' }} />
                ¿Se ha hecho ensayo?
            </label>
            @error('ensayo_hecho')
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            ...
            <label for="plano_provisional" class="text-sm text-gray-700 dark:text-gray-400">
                ...
                <input type="checkbox" id="plano_provisional" value = "1" {{ $participante->imasd != $loged_user->id ? 'disabled' : '' }} name="plano_provisional" @if ($imasd->plano_provisional) checked @endif  class="mr-2 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 " {{ old('plano_provisional') ? 'checked' : '' }} />
                Plano provisional
            </label>
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            ...
            <label for="realizado_prototipo" class="text-sm text-gray-700 dark:text-gray-400">
                ...
                <input type="checkbox" id="realizado_prototipo" name="realizado_prototipo" {{ $participante->imasd != $loged_user->id ? 'disabled' : '' }} @if ($imasd->realizado_prototipo) checked @endif class="mr-2 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" {{ old('realizado_prototipo') ? 'checked' : '' }}/>
                Realizado prototipo
            </label>
        ...
        <div class="form-group relative w-full md:w-1/4 px-3 mb-6 md:mb-0 mt-4">
            ...
            <select id="paso_siguiente" name="paso_siguiente" {{ $participante->imasd != $loged_user->id ? 'disabled' : '' }}>
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <textarea id="notas" name="notas" {{ $participante->imasd != $loged_user->id  ? 'readonly' : '' }} maxlength="500" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500" placeholder=" " >{{ $imasd -> notas}}</textarea>
    ...
    <br/>
    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded"  @if($hdp->rechazado || $hdp->secuencia > 1 || $participante->imasd != $loged_user->id ) style="display:none;" @endif>Enviar</button>
</form>
<br/>
