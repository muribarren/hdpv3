<form method="POST" action="{{ route('procesar')}}" enctype="multipart/form-data">
    ...
    <div class="w-full border border-black rounded-lg p-6 mt-6">
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            ...
            <input type="date" id="revision_serie_fecha" name="revision_serie_fecha" {{ $participante->calidad != $loged_user->id  ? 'readonly' : '' }} value="{{ old('revision_serie_fecha', isset($calidad->revision_serie_fecha) && $calidad->revision_serie_fecha ? \Carbon\Carbon::parse($calidad->revision_serie_fecha)->format('Y-m-d') : '') }}">
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            ...
            <label for="distribucion_copias" class="text-sm text-gray-700 dark:text-gray-400">
                ...
                <input type="checkbox" id="distribucion_copias" value = "1" {{ $participante->calidad != $loged_user->id  ? 'readonly' : '' }} name="distribucion_copias" @if ($calidad->distribucion_copias) checked @endif  class="mr-2 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 " {{ old('distribucion_copias') ? 'checked' : '' }} />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <textarea type="text" id="notas" name="notas" maxlength="500" {{ $participante->calidad != $loged_user->id  ? 'readonly' : '' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  readonly placeholder=" " >{{ $calidad->notas ?? '' }}</textarea>
...
<form id="rechazarForm" action="{{ route('rechazar') }}" method="POST" enctype="multipart/form-data">
    ...
    <button type="button" id="btnRechazar" class="bg-red-600 text-white px-4 py-2 rounded"
        @if($hdp->rechazado || $hdp->secuencia > 7 ||  $participante->calidad != $loged_user->id) style="display:none;" @endif>
        Rechazar
    </button>
</form>
