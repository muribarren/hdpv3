...
<div class="w-full max-w-4xl mx-auto mt-8">
    ...
    <div class="border-b border-gray-300 flex space-x-4" id="tabs">
        ...
        <button onclick="openTab('produccioncompras', this)" @if($hdp->secuencia < 2 || ($hdp->rechazado && $hdp->secuencia == 2)) style="display:none;" @endif class="tab-btn text-gray-600 hover:text-blue-600 hover:border-blue-600 border-b-2 border-transparent py-2 px-4">Producción</button>
        <button onclick="openTab('xxx', this)" @if($hdp->secuencia < 3 || ($hdp->rechazado && $hdp->secuencia == 3)) style="display:none;" @endif class="tab-btn text-gray-600 hover:text-blue-600 hover:border-blue-600 border-b-2 border-transparent py-2 px-4">Compras</button>
        ...
        <div class="w-full border border-black rounded-lg p-6 mt-6">
            ...
            <p>
                ...
                @if($hdp->secuencia >= 2 && isset($participante->imasd))
                    <span class="text-blue-600 font-semibold"> - Validado por departamento I+D. {{ $usuarios->firstWhere('id', $participante->imasd)->name }} {{ $imasd->created_at->format('Y-m-d') }}</span> <br/>
                    @if (!isset($participante->produccion))
                        <n>- Pendiente de validación por Producción</n><br/>
                    @endif
                @endif
                @if($hdp->secuencia >= 3 && isset($participante->produccion))
                    <span class="text-blue-600 font-semibold"> - Validado por departamento Producción. {{ $usuarios->firstWhere('id', $participante->produccion)->name}} {{ $produccion->created_at->format('Y-m-d') }}</span><br/>
                    @if (!isset($participante->compras))
                        <n>- Pendiente de validación por Compras</n><br/>
                    @endif
                @endif
                @if($hdp->secuencia >= 4 && isset($participante->compras))
                    <span class="text-blue-600 font-semibold"> - Validado por departamento Compras. {{ $usuarios->firstWhere('id', $participante->compras)->name }} {{ $compras->created_at->format('Y-m-d') }}</span><br/> 
                    @if (!isset($participante->costos))
                        <n>- Pendiente de validación por Costos</n><br/>
                    @endif
                @endif
                @if($hdp->secuencia >= 5 && isset($participante->costos))
                    <span class="text-blue-600 font-semibold"> - Validado por departamento Costos. {{ $usuarios->firstWhere('id', $participante->costos)->name }} {{ $costos->created_at->format('Y-m-d') }}</span><br/> 
                    @if (!isset($participante->ventas))
                        <n>- Pendiente de validación por Ventas</n><br/>
                    @endif
                @endif    
                @if($hdp->secuencia >= 6 && isset($participante->ventas))
                    <span class="text-blue-600 font-semibold"> - Validado por departamento Ventas. {{ $usuarios->firstWhere('id', $participante->ventas)->name }} {{ $ventas->created_at->format('Y-m-d') }}</span><br/> 
                    @if (!isset($participante->imasd))
                        <n>- Pendiente de validación por I+D</n><br/>
                    @endif
                @endif
                @if($hdp->secuencia >= 7 && isset($participante->imasd2))
                    <span class="text-blue-600 font-semibold"> - Validado por departamento I+D. {{ $usuarios->firstWhere('id', $participante->imasd2)->name }} {{ $imasd2->created_a->format('Y-m-d') }}</span><br/>
                    @if (!isset($participante->calidad))
                        <n>- Pendiente de validación por Calidad</n><br/>
                    @endif
                @endif
                @if($hdp->secuencia >= 8 && isset($participante->calidad))
                    <span class="text-green-600 font-semibold"><br/><n> - HDP COMPLETADO. Validado por departamento de Calidad por {{ $usuarios->firstWhere('id', $participante->calidad)->name }} {{ $calidad->created_at->format('Y-m-d') }}</n></span>
                @endif  
            </p>
