@php
    #$puedeEditar = (in_array($loged_user->id, [2, 3]) || $loged_user->id == $produccion->user_id);
    $puedeEditar = true;
@endphp
...
<form method="POST" action="{{ route('procesar')}}" enctype="multipart/form-data">
    ...
    <div class="w-full border border-black rounded-lg p-6 mt-6">
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            ...
            <label for="inversion_necesaria" class="text-sm text-gray-700 dark:text-gray-400">
                ...
                <input type="checkbox" id="inversion_necesaria" value = "1" {{ $puedeEditar ? '' : 'disabled' }} name="inversion_necesaria" @if ($produccion->inversion_necesaria) checked @endif  class="mr-2 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 " {{ old('inversion_necesaria') ? 'checked' : '' }} />
                Inversion necesaria
            </label>
        ...
        <div id="sustitucionGroup" class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="importe_inversion" name="importe_inversion" maxlength="250" value="{{ $produccion -> importe_inversion}}"  {{ $puedeEditar ? '' : 'readonly' }}  class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="material" name="material" value="{{ $produccion->material ?? '' }}"   maxlength="250"  {{ $puedeEditar ? '' : 'readonly' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="acabado" name="acabado" value="{{ $produccion->acabado ?? '' }}"  maxlength="250"   {{ $puedeEditar ? '' : 'readonly' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="peso_pieza" name="peso_pieza" value="{{ $produccion->peso_pieza ?? '' }}"   maxlength="250"  {{ $puedeEditar ? '' : 'readonly' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="igual_a_estructura" name="igual_a_estructura" value="{{ $produccion->igual_a_estructura ?? '' }}"  maxlength="250"   {{ $puedeEditar ? '' : 'readonly' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            ...
            <input type="date" id="plazo" name="plazo"  {{ $puedeEditar ? '' : 'readonly' }} value="{{ old('plazo', isset($produccion->plazo) && $produccion->plazo ? \Carbon\Carbon::parse($produccion->plazo)->format('Y-m-d') : '') }}">
            @error('plazo')
                <div class="error-message">{{ $message }}</div>
            @enderror       
        </div>
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <textarea type="text" id="notas" name="notas" maxlength="500" {{ $puedeEditar ? '' : 'readonly' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500" placeholder=" " >{{ $produccion->notas ?? '' }}</textarea>
    ...
    @if(!$hdp->rechazado && $hdp->secuencia <= 2 && $puedeEditar)
...
<form id="rechazarForm" action="{{ route('rechazar') }}" method="POST" enctype="multipart/form-data">
    ...
    <button type="button" id="btnRechazar2" class="bg-red-600 text-white px-4 py-2 rounded"
        @if(!(!$hdp->rechazado && $hdp->secuencia <= 2 && $puedeEditar)) style="display:none;" @endif>
        Rechazar
    </button>
</form>
...
<script>
    ...
    document.getElementById('btnRechazar2').addEventListener('click', function() {
