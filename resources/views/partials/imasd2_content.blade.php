<form method="POST" action="{{ route('procesar')}}" enctype="multipart/form-data">
    ...
    <div class="w-full border border-black rounded-lg p-6 mt-6">
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="plan_distribucion" name="plan_distribucion" maxlength="250" value="{{ $imasd2->plan_distribucion ?? '' }}"   {{ $participante->imasd != $loged_user->id  ? 'readonly' : '' }} required class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <label for="establecido_plano" class="text-sm text-gray-700 dark:text-gray-400">
                ...
                <input type="checkbox" id="establecido_plano" value = "1" {{ $participante->imasd != $loged_user->id  ? 'disabled' : '' }} name="establecido_plano" @if ($imasd2->establecido_plano) checked @endif  class="mr-2 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 " {{ old('establecido_plano') ? 'checked' : '' }} />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <label for="revision_plano" class="text-sm text-gray-700 dark:text-gray-400">
                ...
                <input type="checkbox" id="revision_plano" value = "1" {{ $participante->imasd != $loged_user->id  ? 'disabled' : '' }} name="revision_plano" @if ($imasd2->revision_plano) checked @endif  class="mr-2 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 " {{ old('revision_plano') ? 'checked' : '' }} />
                Revisión de plano
            </label>
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            ...
            <input type="date" id="fecha_distribucion" name="fecha_distribucion" {{ $participante->imasd != $loged_user->id  ? 'readonly' : '' }} value="{{ old('fecha_distribucion', isset($imasd2->fecha_distribucion) && $imasd2->fecha_distribucion ? \Carbon\Carbon::parse($imasd2->fecha_distribucion)->format('Y-m-d') : '') }}">
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="codigo_comercial" name="codigo_comercial" maxlength="250" value="{{ $imasd2->codigo_comercial ?? '' }}"    {{ $participante->imasd != $loged_user->id  ? 'readonly' : '' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="semielaborados" name="semielaborados" maxlength="250" value="{{ $imasd2->semielaborados ?? '' }}"    {{ $participante->imasd != $loged_user->id  ? 'readonly' : '' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <textarea type="text" id="notas" name="notas" maxlength="500" {{ $participante->imasd != $loged_user->id  ? 'readonly' : '' }} class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  readonly placeholder=" " >{{ $imasd2->notas ?? '' }}</textarea>
    ...
    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded" @if($hdp->rechazado || $hdp->secuencia > 6 ||  $participante->imasd != $loged_user->id) style="display:none;" @endif>Enviar</button>
