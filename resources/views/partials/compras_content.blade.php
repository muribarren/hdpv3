@php
    #$puedeEditar = (in_array($loged_user->id, [2, 3]) || $loged_user->id == $compras->user_id);
    $puedeEditar = true;
@endphp
...
<form method="POST" action="{{ route('procesar')}}" enctype="multipart/form-data">
    ...
    <div class="w-full border border-black rounded-lg p-6 mt-6">
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="precio" name="precio" value="{{ $compras->precio ?? '' }}"  maxlength="250"  {{ $puedeEditar ? '' : 'readonly' }} required class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <input type="text" id="inversion_troquel" name="inversion_troquel" maxlength="250" value=" {{ $compras->inversion_troquel ?? '' }}"    {{  $puedeEditar  ? '' : 'readonly' }}  class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500"  placeholder=" " />
        ...
        <div class="form-group relative w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-4">
            <textarea type="text" id="notas" name="notas" {{  $puedeEditar ? '' : 'readonly' }} maxlength="500" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer bg-gray-100 text-gray-500" placeholder=" " >{{ $compras->notas ?? '' }}</textarea>
    ...
    @if(!$hdp->rechazado && $hdp->secuencia <= 3 && $puedeEditar)
